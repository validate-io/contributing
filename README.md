...
[Validate.io](https://github.com/validate-io/overview) is a collection of JavaScript validation utilities. Each utility is written as a standalone module which can be independently imported into any application or project. 
...
Validate.io is guided by three design principles: __composition__, __consistency__, and __rigor__.

* 	__Composition__: standalone modules encourage flexible composition (build your own validator), constrained design (do one thing; do one thing well), and restraint (only use what you need).

*	__Consistency__: all modules should look and feel as if written by a single author. A user should be free to focus on understanding implementations without the distraction of arbitrary changes in style.

*	__Rigor__: testing and benchmarking should be priorities, not afterthoughts, and implementations should be robust.

What follows is a guide to contributing to [Validate.io](https://github.com/validate-io).


## Joining

Have a great idea for a validation module or want to become more involved? Awesome. :)

You have a couple of options...

1. 	Create a module linked to your personal Github account and submit a pull request for inclusion in the main [validate.io](https://github.com/validate-io/validate.io) library.
2. 	(__preferred option__) Ask an [owner](https://github.com/kgryte) to become a [validate-io](https://github.com/validate-io) member.

If you opt for option #1, note that all validation modules are expected to follow the same style guide described below.
...
All validation modules should follow the same JavaScript [style guide](https://github.com/kgryte/javascript-style-guide). Users should not have to decode styles to read and understand what matters: implementations.
...
function hasLength( value, length ) {
	if ( typeof length !== 'number' || length !== length || length%1 !== 0 || length < 0 ) {
		throw new TypeError( 'hasLength()::invalid input argument. Length must be an integer greater than or equal to 0. Value: `' + length + '`.' );
	}
	// Do something...
}
...
Contributors are encouraged to use the [Yeoman](http://yeoman.io/) module [generator](https://github.com/validate-io/generator-validate-io). The generator automates many aspects of module generation by creating a base scaffold from which to build a validation module.

A couple of notes...

1. 	Before using the generator, be sure to create a remote repository on [Github](https://github.com/validate-io). The generator will use the repository name to generate the remote URLs included in the `package.json` and `README.md`. Additionally, the generator takes care of setting the remote origin for the local Git repository, so you can begin pushing code to the [validate-io](https://github.com/validate-io) organization immediately after generation.
...
The following is a typical workflow when creating validation modules...

1. 	Create a repository within the Github [organization](https://github.com/validate-io/).
...
4. 	Run the [generator](https://github.com/validate-io/generator-validate-io) and follow the prompts.
	
	``` bash
	$ yo validate-io
	```

	Note that the module name should follow the convention of the repository name being prefixed with `validate.io-`; .e.g., `validate.io-integer`, where `integer` is the repository name.
...
If you use the Validate.io [generator](https://github.com/validate-io/generator-validate-io), the above test modules are included along with stubbed test code. All modules should have __100%__ code coverage.
